  ...
  return (
    ...
    >
      ...
      >
          ...
          {(!isCollapsed || isHovered) && links.map(link => (
            <Flex mx={4} alignItems="center" justifyContent={"center"} key={link.url}>
              <Link href={link.url} variant="ghost">
                <Text as="a" fontWeight={currentPage === link.url ? 'bold' : 'normal'} display="flex" flexDir="column" justifyContent="center">
                  {link.name}
                </Text>
              </Link>
            </Flex>
        ...
        {(!isCollapsed || isHovered) && <Flex alignItems="center" justifyContent={"center"} position={["fixed", "inherit"]} bottom="24px" right="24px">
          <IconButton 
            _focus={{
